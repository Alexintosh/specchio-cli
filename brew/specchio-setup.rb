...
class SpecchioSetup < Formula
  ...
  def install
    bin.install "cli/specchio-setup" => "specchio-setup"
  end

  test do
    assert_match "Specchio Setup CLI", shell_output("#{bin}/specchio-setup --version", 1)
    assert_match "Interactive setup CLI for Specchio", shell_output("#{bin}/specchio-setup --help", 1)
  end
end
